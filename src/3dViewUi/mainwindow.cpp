...
void MainWindow::createScreenshot() {
  ...
  if (ui->btn_screen_bmp) {
    ui->OGLwidget->grab().save("../../../../screenshot/" + currentDateTime +
                               ".bmp");
    ...
    for (int i = 0; i <= 50; i++) {
      ui->OGLwidget->grab().save("../../../../screenshot/gif_obj" +
                                 QString::number(i) + ".jpg");
    }
    QString fileName =
        "convert -delay 10 -loop 0 ../../../../screenshot/gif_obj/*.jpg ../" +
        currentDateTime + ".gif";
    QByteArray ba = fileName.toLocal8Bit();
    system(ba);
  }
...
void MainWindow::updateUiColors() {
  ui->widgetBackGroundColor->setAutoFillBackground(true);
  ui->widgetBackGroundColor->setStyleSheet(
